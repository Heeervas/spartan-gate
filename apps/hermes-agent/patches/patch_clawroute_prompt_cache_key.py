...
MARKER = "# spartan-gate: clawroute prompt cache key v2"
LEGACY_MARKER = "# spartan-gate: clawroute prompt cache key"
...
def patch_source(source: str) -> tuple[str, bool]:
    ...
    helper = [
        MARKER,
        "def _spartan_add_clawroute_prompt_cache_key(api_kwargs, params):",
        ...
        "    if session_id and clawroute_base_url and request_base_url.rstrip('/') == clawroute_base_url.rstrip('/'):",
...
def patch_file(path: Path) -> str:
    ...
    return f"Patched: {path} sends session_id as prompt_cache_key to ClawRoute"
